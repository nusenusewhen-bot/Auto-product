...
async function getAddressState(address) {
  try {
    ...
    if (!data?.data?.[address]) return { confirmed: 0, unconfirmed: 0, total: 0, txs: [] };
    ...
    return { confirmed: 0, unconfirmed: 0, total: 0, txs: [] };
  }
}
...
client.once('ready', async () => {
  ...
  // Fast monitoring every 10 seconds for 0-conf detection
  setInterval(monitorMempool, 10000);
  // Slower confirmation check every 30 seconds
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'panel') {
      ...
      await interaction.editReply({ content: '⚠️ Send feature requires manual implementation with private keys.' });
    }
  }
